...
internal partial class MyTableUIElement : Panel
{
    ...
    private IEnumerable<FrameworkElement> ContentChildren
    {
        get
        {
            for (int i = _columnCount + _rowCount - 1; i < Children.Count; i++)
            {
    ...
    private IEnumerable<Rectangle> HorizontalLines
    {
        get
        {
            for (int i = _columnCount; i < _columnCount + _rowCount - 1; i++)
            {
                yield return (Rectangle)Children[i];
            }
        }
    ...
    protected override Size ArrangeOverride(Size finalSize)
    {
        ...
        foreach (FrameworkElement child in ContentChildren)
        {
            ...
            Rect rect = new(_borderThickness, 0, 0, 0);
        ...
        {
            border.Arrange(new Rect(new(0, 0), size));
        }
